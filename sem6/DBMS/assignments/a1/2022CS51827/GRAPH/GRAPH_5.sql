...
all_paths AS (
    SELECT subject_id1 AS start_id, subject_id2 AS connected_id, 1 AS path_length
    FROM directed_edge_list
    WHERE subject_id1 = 10037861
    
    UNION ALL

    SELECT ap.start_id, de.subject_id2, ap.path_length + 1
    FROM all_paths ap
    JOIN directed_edge_list de ON ap.connected_id = de.subject_id1
    WHERE ap.path_length < 20
)
