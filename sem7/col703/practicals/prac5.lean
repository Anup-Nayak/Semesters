...
-- YOUR ANSWER HERE
theorem gt_4 : ∀ a : Nat, a > 4 → (a > 0 ∧ a > 1 ∧ a > 2 ∧ a > 3) :=
  by
    grind
...
-- YOUR ANSWER HERE
theorem gt_n : ∀ m n: Nat, m > n → ∀ a : Nat, a <= n → m > a :=
  by
    grind
...
-- YOUR ANSWER HERE
theorem gt_4_repeat : ∀ a : Nat, a > 4 → (a > 0 ∧ a > 1 ∧ a > 2 ∧ a > 3) :=
by
  intro a h
  repeat' apply And.intro
  repeat' apply Nat.lt_trans (by decide) h
...
theorem nextWorkingDay_Monday :
  nextWorkingDay day.friday = day.monday ∧
  nextWorkingDay day.saturday = day.monday ∧
  nextWorkingDay day.sunday = day.monday :=
  by
  repeat' apply And.intro
  repeat' rfl
