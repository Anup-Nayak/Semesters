...
-- Helper theorem: reversing an append
theorem myRev_append {α : Type} (xs ys : List α) :
...
theorem myrev_idemp {α : Type} (xs : List α) : myRev (myRev xs) = xs := by
  -- match xs with
  -- | List.nil =>
  --   simp [myRev]
  -- | List.cons x tl =>
  --   simp [myRev, myRev_append]
  --   rw [myrev_idemp tl]
...
theorem eqlist_eqcomp {α : Type} :
  ∀ {a b : α} {t1 t2 : List α}, (a :: t1) = (b :: t2) → a = b ∧ t1 = t2 :=
by
  intro a b t1 t2 h
  cases h
...
def uniq {α : Type} [BEq α] : List α → Bool
  | []       => true
  | h :: t   => if t.contains h then false else uniq t
...
theorem len_sup_long_correct :
  ∀ (α : Type) [BEq α] (l1 l2 : List α),
    uniq l1 = true →
    (∀ x, l1.contains x = true → l2.contains x = true) →
    l1.length ≤ l2.length :=
by
  intro α _ l1 l2 huniq hsub
  sorry
