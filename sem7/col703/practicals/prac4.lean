...
-- YOUR ANSWER GOES HERE
theorem in_append : ∀ {α : Type} {x : α} {l1 l2 : List α}, BelongsTo x l1 → BelongsTo x (l1 ++ l2) :=
by
  intro α x l1 l2 h
  induction h with
  | isHead tl =>
...
-- YOUR ANSWER GOES HERE
theorem in_append2 : ∀ {α : Type} {x : α} {l1 l2 : List α}, BelongsTo x l2 → BelongsTo x (l1 ++ l2) :=
by
  intro α x l1 l2 h
  induction l1 with
  | nil =>
...
inductive uniq_ind {α : Type} [BEq α] : List α → Prop
  | nil : uniq_ind []
  | cons : ∀ (hd : α) (tl : List α),
      ¬ BelongsTo hd tl → uniq_ind tl → uniq_ind (hd :: tl)
...
-- Converse is false - l1 = {1,2,3} l2 = {1,2,3} then l1++l2 = {1,2,3,1,2,3} <- not unique

theorem not_belongs {α : Type} (x : α) (l1 l2 : List α) : ¬BelongsTo x (l1 ++ l2) → ¬BelongsTo x l1 :=
by
  intro h hl1
  apply h
  exact @in_append α x l1 l2 hl1

theorem uniq_suffix {α : Type} [BEq α] (x : α) (l : List α) : uniq_ind (x :: l) → uniq_ind l :=
by
  intro h
  cases h with
  | cons _ _ _ huniq => exact huniq

theorem uniq_sublists {α : Type} [BEq α] (l1 l2 : List α) : uniq_ind (l1 ++ l2) → uniq_ind l1 ∧ uniq_ind l2 :=
by
  intro h
  induction l1 with
  | nil =>
      ...
      · exact h
  | cons hd tl Ih =>
      have htail := uniq_suffix hd (tl ++ l2) h
      have ⟨hl1, hl2⟩ := Ih htail
      cases h with
      | cons _ _ hnotin _ =>
          constructor
          · exact uniq_ind.cons hd tl (not_belongs hd tl l2 hnotin) hl1
          · exact hl2
